...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Tchauzinhu amigo(a) @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛AGUARDE AMIGO(A)⌛',
				success: '✔️SUCESSO✔️',
				error: {
					stick: '❌FALHA AO TENTAR CONVERTER❌',
					Iv: '❌LINK INVÁLIDO❌'
				},
				only: {
					group: '❌ COMANDO SOMENTE PARA GRUPO ❌',
					ownerG: '❌ESSE COMANDO SÓ PODE SER USADA PELO GRUPO PROPRIETÁRIO❌',
					ownerB: '❌ESSE COMANDO SÓ PODE DER USADO PELO CRIADOR DO BOT❌',
					admin: '❌COMANDO PERMITIDO SOMENTE PARA ADMS❌',
					Badmin: '❌ COMANDO SÓ PODE SER USADO QUANDO O BOT É ADEMAR DO GRUPO❌'
				}
			}
			...
			switch(command) {
				...
				case 'info':
					...
					break
				case 'blocklist':
					teks = 'lista de número bloqueados :\n'
				...
				case 'gtts':
					...
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							...
							if (err) return reply('falhoul om:(')
				...
				case 'url2img':
					...
					if (args.length < 2) return reply('cade o link gay?')
				...
				case 'tstiker':
				case 'tsticker':
					if (args.length < 1) return reply('cade o texto gay?')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
				...
				case 'add':
					...
					if (args.length < 1) return reply('Você quer adicionar um gênio? fala o número animal')
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Membro banido : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
					if (!isGroup) return reply(mess.only.group)
					teks = `Todos adms desse grupo *${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
					...
					break
				case 'toimg':
					if (!isQuotedSticker) return reply('seleciona uma figurinha')
					reply(mess.wait)
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('❌ Falha ao converter figurinha em imagen ❌')
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Modo simi ativado com sucesso neste grupo ✔️')
